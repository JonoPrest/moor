...
- **Pending vs committed events are different types.** `client-core` has `PendingEvent { client_seq, .. }` and `CommittedEvent { seq, .. }`; you cannot render a `Seq` that doesn't exist.
...
- **Sans-I/O boundaries** make this checkable: `client-core` returns `Vec<Effect>`; a test asserts exactly which effects, no more.

### Testing

Each crate has its own test strategy, listed per milestone. Common tools: `insta` for snapshots, `proptest` for invariants, `tempfile` + real `git` for repo fixtures, `cargo nextest`. A shared `test-support` crate provides repo builders (`RepoBuilder::new().commit("a", files![...]).branch("x")...`).

CI: `cargo nextest run`, `cargo clippy -D warnings`, `cargo check -p protocol -p client-core --target wasm32-unknown-unknown`, `cargo fmt --check`, ReScript build + tests.

---

## Milestone 1 — `protocol` + `review-core`
...
- Cargo workspace; crates `protocol`, `review-core`, `test-support`, `xtask`.
- Lints: `#![deny(missing_debug_implementations)]`, clippy pedantic subset, `wildcard_enum_match_arm` in `review-core`/`client-core`.
- CI as above.
...
### 1.3 Event store (`review-core::store`)
...
### 1.4 Git engine (`review-core::git`)
...
- Tests with `test-support` repos: each `RefSpec` variant resolves; renames detected; working-tree snapshot reflects unstaged edits and untracked files; binary files flagged.

### 1.5 Diff + render model (`review-core::render`)
...
### 1.6 Reviews (`review-core::review`)
...
### 1.7 Comments + anchoring (`review-core::comments`)
...
### 1.8 `Core` façade
- One `Core` struct composing the above; every public method takes/returns `protocol` types; this is the surface every transport adapts.
...
Goal: `cr-daemon` running, multiple clients connected, events streaming, MCP working.
...
### 2.5 MCP
- `cr-mcp` stdio binary proxying to daemon socket; tool per `Core` method; `Author::Agent` from MCP client info + session.
...
### 2.6 `cr` CLI
- `cr workspace add/list`, `cr review create --base --head [--repo ...]`, `cr comment ...`, `cr events --follow`. Same client lib as MCP.
- Tests: `assert_cmd` against a spawned daemon in a temp dir.

### 2.7 Lifecycle
- Data dir, socket path, `cr-daemon --stdio` mode for `ssh host cr-daemon --stdio`, graceful shutdown, crash-safe reopen.
- Tests: kill -9 mid-append → reopen consistent (redb guarantees; assert view rebuild path works).

---

## Milestone 3 — `client-core`
...
Exit criteria: wasm target check passes for `client-core`; simulator suite green.
...
## Later
- Browser build (`client-wasm`, daemon serves `ui/`), TUI, cross-machine sync, GitHub export.
