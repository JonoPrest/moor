...
- **Contexts** (`moor-config`, `~/.config/moor/config.toml`): named places a daemon lives — `Local{data_dir?, socket?}`, `Ssh{host, moord?, args}`, `Ws{url}` — plus which one is current, kubectl-style. Every client (CLI, MCP, desktop app) resolves a context the same way: ad-hoc flags (`--socket`, `--ws`) > `--context`/`MOOR_CONTEXT` > `current` > an implicit `local`. `moor context add-local|add-ssh|add-ws|list|use|show|remove`.
...
| 18 | Daemon lifecycle | one daemon per machine; `moord --stdio` is a proxy that auto-starts it; clients hold named contexts (local/ssh/ws) and can probe/start/stop; ws contexts are unmanaged | herdr-style remotes without a second store opener; kubectl-style switching for the app |
