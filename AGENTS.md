...
- Exhaustive matching on domain enums — no `_ =>` arms in `review-core`/`client-core`
  (clippy `wildcard_enum_match_arm` is on).
- Sans-I/O cores (`client-core`) return `Vec<Effect>`; tests assert exactly which effects.
...
- Real git repos via `test-support::RepoBuilder` — no mocked git.
...
- `client-core` behaviour under races is tested with the two-client simulator.

## Conventions

- `protocol` and `client-core` must stay wasm-safe: no tokio, std I/O, threads,
...
- Transports (`unix`, `ws`, `mcp`, `cli`) are thin adapters over `Core`; never add a
