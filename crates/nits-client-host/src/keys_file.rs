//! `~/.config/nits/keys.toml` → the typed keys config (UI-DESIGN: modal
...
//! they surface in the help overlay and `nits keys check`.

use std::path::PathBuf;

use nits_client_core::{Keymap, KeysConfig};
...
pub enum KeysFileError {
    ...
    #[error("keys.toml: {0}")]
    Config(#[from] nits_client_core::KeysError),
}

/// `$XDG_CONFIG_HOME/nits/keys.toml` or `~/.config/nits/keys.toml`.
#[must_use]
pub fn default_keys_path() -> Option<PathBuf> {
    if let Ok(x) = std::env::var("XDG_CONFIG_HOME") {
        return Some(PathBuf::from(x).join("nits").join("keys.toml"));
    }
    std::env::var("HOME")
        .ok()
        .map(|home| PathBuf::from(home).join(".config/nits/keys.toml"))
}
...
pub fn default_file() -> String {
    use nits_client_core::{Mode, config_name, modes_of};
    use std::fmt::Write as _;
    use strum::IntoEnumIterator;
    let map = Keymap::default_table();
    let mut out = String::from(
        "#:schema ./keys.schema.json\n\
         # nits keys — generated by `nits keys init`. Every action appears\n\
         # under its mode with its default sequences; edit freely. `[]`\n\
         # unbinds. `<leader>` is the leader chord below.\n\nleader = \"space\"\n",
    );
    for mode in Mode::iter() {
        let _ = write!(out, "\n[bindings.{}]\n", mode.to_string().to_lowercase());
        for command in nits_client_core::Command::iter() {
            if !modes_of(command).contains(&mode) {
                continue;
            }
            let in_mode = |ctx: nits_client_core::Context| match mode {
                Mode::Insert => ctx == nits_client_core::Context::Composer,
                Mode::Normal => ctx != nits_client_core::Context::Composer,
                // Visual reuses the Diff-context rows (keymap `with_config`).
                Mode::Visual => ctx == nits_client_core::Context::Diff,
            };
...
pub fn schema_json() -> String {
    use nits_client_core::{Command, Mode, config_name, label, modes_of};
...
mod tests {
    use super::*;
    use nits_client_core::{Command, Context, KeyChord, Lookup, NamedKey};
    ...
    fn rejects_unknown_wrong_mode_and_bad_chords_loudly() {
        assert!(matches!(
            ...
            KeysFileError::Config(nits_client_core::KeysError::UnknownAction(a)) if a == "frobnicate"
        ));
        assert!(matches!(
            parse("[bindings.insert]\ncomment = [\"c\"]\n").unwrap_err(),
            KeysFileError::Config(nits_client_core::KeysError::WrongMode { .. })
        ));
        assert!(matches!(
            parse("[bindings.normal]\nopen = [\"ctrl+bogus+x\"]\n").unwrap_err(),
            KeysFileError::Config(nits_client_core::KeysError::BadKeys { .. })
        ));
    ...
    fn default_file_round_trips_and_covers_every_action() {
        ...
        let normal = config
            .bindings
            .get(&nits_client_core::Mode::Normal)
            .unwrap();
        for command in Command::iter() {
            if nits_client_core::modes_of(command).contains(&nits_client_core::Mode::Normal) {
                assert!(
                    normal.contains_key(&nits_client_core::config_name(command)),
