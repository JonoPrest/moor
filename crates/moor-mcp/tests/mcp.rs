...
use moor_mcp::jsonrpc::{Incoming, Outgoing};
use moor_mcp::server::AgentIdentity;
use moor_mcp::{Endpoint, Server};
use moor_protocol::{Author, BuildInfo, ClientId, ClientSeq, Human, Mutation, Request, Response};
...
async fn seed(h: &Harness, c: &Client) -> (String, String) {
    let ws = moor_protocol::WorkspaceId::from_parts(1, 1);
    let rid = moor_protocol::RepoId::from_parts(1, 1);
    for (n, m) in [
...
async fn tools_list_is_json_rpc_conformant() {
    ...
    assert_eq!(r["serverInfo"]["name"], json!("moor-mcp"));
...
async fn subscribe_events_long_polls_and_resumes() {
    ...
    .await;
    let review_id = created["review"]["id"].as_str().unwrap().to_string();
    let rid_typed: moor_protocol::ReviewId = review_id.parse().unwrap();
