...
use moor_client_core::{IdSeed, ViewPatch};
use moor_client_host::{Identity, KvConfig, local_config};
use moor_protocol::{Author, BuildInfo, ClientId, ViewSection};
...
async fn browser_attach_sees_subscribed_view() {
    ...
    let socket = dir.path().join("moord.sock");
    ...
    let bridge = moor_client_web::serve(SocketAddr::from((Ipv4Addr::LOCALHOST, 0)), config)
        .await
        .unwrap();
