...
use moor_client_core::{Action, Effect};
use moor_protocol::{
...
use moor_test_support::{Divergence, Peer, Sim};
...
fn thread_id(sim: &Sim) -> moor_protocol::ThreadId {
...
fn resolve_unresolve_race_converges_and_the_loser_is_undone() {
    ...
    assert!(matches!(
        err,
        moor_client_core::CoreError::Mutation(moor_client_core::MutationError::AlreadyResolved(_))
    ));
    ...
    assert!(matches!(
        err,
        moor_client_core::CoreError::Mutation(moor_client_core::MutationError::NotResolved(_))
    ));
    sim.settle();
    sim.converged().unwrap();
}
                ...
                Step::Down(p) => {
                    ...
                    if let Some(Err(e)) = sim.deliver_down(p) {
                        prop_assert!(
                            matches!(
                                e,
                                moor_client_core::CoreError::UnknownRequest(_)
                                    | moor_client_core::CoreError::StaleEvent { .. }
                            ),
