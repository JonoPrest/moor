...
//! threshold. `NITS_BENCH_FILES` scales the repo (default 50 000; CI uses a
...
use nits_test_support::{TestRepo, synthetic_files, synthetic_repo};

const RUNS: usize = 5;

fn files() -> usize {
    std::env::var("NITS_BENCH_FILES")
        .ok()
...
fn comment_burst(repo: &TestRepo) -> Line {
    ...
    let core = Core::open(&DataDir::new(dir.path().join("nits"))).expect("core");
    let c = ctx();
