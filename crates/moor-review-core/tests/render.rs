...
use moor_protocol::{ChunkIndex, ExpandDir, RenderContent, RenderOpts, Row};
use moor_review_core::render::{CHUNK_ROWS, Highlighter, Rendered, render_blob, render_file};
...
fn check_invariants(r: &Rendered, old: Option<&[u8]>, new: Option<&[u8]>) {
    ...
    for row in &r.rows {
        let cells: Vec<(&moor_protocol::Cell, bool)> = match row {
