...
pub enum StoreError {
    #[error(
        "store was written by a newer moord (schema {found}, this build supports up to {supported})"
    )]
    SchemaTooNew {
        found: SchemaVersion,
...
impl Store {
    ...
    pub fn comments_on_blob(
        ...
        repo: RepoId,
        blob: BlobOid,
    ) -> Result<Vec<(ReviewId, moor_protocol::CommentId)>, StoreError> {
        let txn = self.db.begin_read()?;
        ...
        for entry in table.range(lo..hi)? {
            ...
            let corrupt = |err: moor_protocol::ParseIdError| StoreError::Corrupt {
                seq: Seq::new(0),
