...
fn unindex_anchor(t: &mut Write<'_>, comment: &moor_protocol::Comment) -> Result<(), StoreError> {
    match &comment.anchor {
        Anchor::Review => {}
        Anchor::File {
            repo_id, blob_oid, ..
...
pub(super) fn apply(t: &mut Write<'_>, event: &Event) -> Result<(), StoreError> {
    ...
    match &event.body {
        ...
        EventBody::WorkspaceUpdated { workspace_id, name } => {
            ...
            let mut ws: moor_protocol::Workspace = get(t.workspaces.get(key.as_str())?)?
...
fn load_comment(
    ...
    seq: Seq,
    review: ReviewId,
    id: moor_protocol::CommentId,
) -> Result<moor_protocol::Comment, StoreError> {
    get(t
        .comments
...
fn save_comment(t: &mut Write<'_>, c: &moor_protocol::Comment) -> Result<(), StoreError> {
    t.comments.insert(
