...
use nits_review_core::store::Store;
...
fn short_socket(tag: &str) -> PathBuf {
    std::env::temp_dir().join(format!("nitsd-{tag}-{}.sock", std::process::id()))
}
...
async fn stdio_proxies_to_an_autostarted_daemon_and_shutdown_stops_it() {
    ...
    assert!(
        nitsd::launch::is_listening(&socket).await,
    ...
    let start = Instant::now();
    while nitsd::launch::is_listening(&socket).await {
        assert!(
            start.elapsed() < Duration::from_secs(10),
            "daemon did not exit"
        );
