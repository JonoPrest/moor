...
async fn mutate(c: &Client, seq: u64, m: Mutation) -> Result<moor_protocol::Event, RpcError> {
        ...
        .await
    {
        ...
        Err(moord::client::ClientError::Rpc(e)) => Err(e),
        Err(e) => panic!("{e}"),
    }
}
...
async fn handshake_reports_versions_and_rejects_unknown_major() {
    ...
    assert!(matches!(
        err,
        moord::client::ClientError::Rejected(RpcError::UnsupportedProtocol { .. })
    ));
}
...
async fn frame_with_wrong_version_after_handshake_is_an_error() {
    ...
    .await
    .unwrap();
    let _welcome = moord::codec::read_msg::<_, moor_protocol::ServerMsg>(&mut raw)
        .await
        .unwrap()
        .unwrap();
    moord::codec::write_msg(
    ...
    .await
    .unwrap();
    let reply = moord::codec::read_msg::<_, moor_protocol::ServerMsg>(&mut raw)
        .await
        .unwrap()
        .unwrap();
...
async fn reanchoring_many_comments_does_not_block_reads(t: Transport) {
    ...
    let blob: moor_protocol::BlobOid = blob.parse().unwrap();
    for n in 0..500u128 {
        let line = u32::try_from(n).unwrap() + 2;
        let anchor = moor_review_core::comments::lines_anchor(
            rid(),
            RepoPath::new("a.rs").unwrap(),
            moor_protocol::Side::Head,
            blob,
            line,
            line,
        )
...
async fn agent_cannot_mark_viewed_and_errors_carry_request_id() {
    ...
    assert!(matches!(
        err,
        moord::client::ClientError::Rpc(RpcError::NotFound { .. })
    ));
}
...
async fn stale_socket_file_is_replaced_and_live_one_is_refused() {
    let dir = tempfile::tempdir().unwrap();
    let path = std::env::temp_dir().join(format!("moord-{}.sock", ulid_ish()));
