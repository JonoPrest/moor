...
use nits_protocol_fixtures::{all, registry};
...
fn unknown_fields_are_rejected() {
    ...
    assert!(serde_json::from_value::<nits_protocol::Anchor>(v).is_err());

    let mut v = serde_json::json!({"ignore_whitespace": false, "context_lines": 3});
    v["extra"] = serde_json::json!(1);
    assert!(serde_json::from_value::<nits_protocol::RenderOpts>(v).is_err());
}
